...
| 상수 | 값 | 의미 |
|------|-----|------|
| `ADC_SAMPLE_MS` | 2 | `k_timer` 주기(ms) — `k_msleep` 대신 타이머로 샘플 트리거 |
| `ADC_WINDOW_MS` | 2000 | 한 번의 min/max **집계 윈도우 길이**(ms) |
| `ADC_CHANNEL_COUNT` | `CONFIG_SMARTGATEWAY_ADC_CHANNEL_COUNT` (기본 8) | 실제 변환하는 채널 수 |

**윈도우 종료:** `window_sample_count >= (ADC_WINDOW_MS / ADC_SAMPLE_MS)` (기본 1000샘플 = 2s).  
타이머 지연으로 `adc_read`가 2ms를 넘기면 세마포가 쌓이며, 루프에서 연속 처리합니다.
...
3. **`elapsed = window_sample_count * 2`** 가 **`2000` 이상**이면:
   - 시각·밀리초 등 스냅샷 메타 갱신.
   - 각 채널: `raw`·`min_val`·`max_val`을 **매 샘플** 볼트로 갱신(UDP 20ms가 읽음).
     - 2초 경과 시 **다음 윈도우**용 `running_min[i] = v`, `running_max[i] = v` 로 리셋.
